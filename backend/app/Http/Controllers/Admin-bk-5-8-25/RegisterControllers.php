...
//            }
    ...
    public function view($id)
    {
        ...
        $category = Category::select('id', 'name')->where('status', 1)->orderBy('name', 'asc')->latest()->get();
        $categories = Categories::select('id', 'name')->where('status', 1)->orderBy('name', 'asc')->latest()->get();
        $models = Models::select('id', 'name')->where('status', 1)->orderBy('name', 'asc')->latest()->get();
//        $data = Register::findOrFail($id);
        ...
        $parts = Parts::select('id', 'name')->where('status', 1)->orderBy('name', 'asc')->latest()->get();
        $business_data = Business::where('user_id', $id)->first();
        
        $review_data = Review::select('*')->where('user_id', $id)->orderBy('id', 'desc')->latest()->get();
...
//        return view('admin.register.view', compact('data','business_data','catalogue_data','category','categories','models','business_shop_images_data','business_company_data','parts','review_data', 'register_names', 'buyer_names', 'seller_names'));
    ...
    public function delete(Request $request)
    {
        ...
        try {
            $exist_user = Register::where('id', request('id'))->limit(1)->first();
            if ($exist_user) {
