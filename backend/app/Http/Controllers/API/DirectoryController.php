...
//                ->where('register.hide_profile_in_directory', '=', 0)
                    ...
                    ->where('register.role_id', '=', $role_id);

            if ($role_id == 1) { // Rental Company
                $query->where(function ($q) use ($search) {
                    $q->orWhere('business.product_info', 'LIKE', '%"product_name":"' . $search . '%')
                            ->orWhere('business.product_info', 'LIKE', '%"company_name":"' . $search . '%')
                            ->orWhere('business.product_info', 'LIKE', '%"model_name":"' . $search . '%')
                            ->orWhere('register.name', 'LIKE', "%{$search}%")
                            ->orWhere('register.personal_name', 'LIKE', "%{$search}%")
                            ->orWhere('business.name', 'LIKE', "%{$search}%");
                });
            } else if ($role_id == 2) { // Dealer/Supplier
                $query->where(function ($q) use ($search) {
                    $q->orWhere('register.service_center_info', 'LIKE', '%"company_name":"' . $search . '%')
                            ->orWhere('register.name', 'LIKE', "%{$search}%")
                            ->orWhere('register.personal_name', 'LIKE', "%{$search}%")
                            ->orWhere('business.name', 'LIKE', "%{$search}%")
                            ->orWhere('business.spare_part_info', 'LIKE', '%"parts_name":"' . $search . '%')
                            ->orWhere('business.spare_part_info', 'LIKE', '%"company_name":"' . $search . '%')
                            ->orWhereExists(function ($q1) use ($search) {
                                $q1->select(DB::raw(1))
                                ->from('categories')
                                ->whereRaw("FIND_IN_SET(categories.id, business.companies_id)")
                                ->where('categories.name', 'like', '%' . $search . '%');
                            })
                            ->orWhereExists(function ($q2) use ($search) {
                                $q2->select(DB::raw(1))
                                ->from('category')
                                ->whereRaw("FIND_IN_SET(category.id, register.category_id)")
                                ->where('category.name', 'like', '%' . $search . '%');
                            })
                            ->orWhereExists(function ($q3) use ($search) {
                                $q3->select(DB::raw(1))
                                ->from('sub_category')
                                ->whereRaw("FIND_IN_SET(sub_category.id, register.sub_category_id)")
                                ->where('sub_category.name', 'like', '%' . $search . '%');
                            });
                });
            } else if ($role_id == 3) { // Manufacturer
                $query->where(function ($q) use ($search) {
                    $q->orWhere('register.service_center_info', 'LIKE', '%"company_name":"' . $search . '%')
                            ->orWhere('register.name', 'LIKE', "%{$search}%")
                            ->orWhere('register.personal_name', 'LIKE', "%{$search}%")
                            ...
                            ->orWhereExists(function ($q1) use ($search) {
                                $q1->select(DB::raw(1))
                                ->from('category')
                                ->whereRaw("FIND_IN_SET(category.id, business.category_id)")
                                ->where('category.name', 'like', '%' . $search . '%');
                            });
                });
                ...
                $query->where(function ($q) use ($search) {
                    $q->orWhere('register.name', 'LIKE', "%{$search}%")
                            ->orWhere('business.name', 'LIKE', "%{$search}%")
                            ->orWhere('register.personal_name', 'LIKE', "%{$search}%")
                            ->orWhere('business.working_with', 'LIKE', "%{$search}%");
                });
            } else if ($role_id == 6) { // Spare Part
                $query->where(function ($q) use ($search) {
                    $q->orWhere('business.spare_part_info', 'LIKE', '%"parts_name":"' . $search . '%')
                            ->orWhere('business.spare_part_info', 'LIKE', '%"company_name":"' . $search . '%')
                            ->orWhere('register.name', 'LIKE', "%{$search}%")
                            ->orWhere('register.personal_name', 'LIKE', "%{$search}%")
                            ->orWhere('business.name', 'LIKE', "%{$search}%")
            ...
            } else if ($role_id == 8) { // Service center
                $query->where(function ($q) use ($search) {
                    $q->orWhere('register.service_center_info', 'LIKE', '%"company_name":"' . $search . '%')
                            ->orWhere('register.name', 'LIKE', "%{$search}%")
                            ->orWhere('business.name', 'LIKE', "%{$search}%")
            ...
            } else if ($role_id == 9) { // Sound Academy
                $query->where(function ($q) use ($search) {
                    $q->orWhere('register.name', 'LIKE', "%{$search}%")
                            ->orWhere('business.name', 'LIKE', "%{$search}%")
                            ->orWhere('register.personal_name', 'LIKE', "%{$search}%")
                            ->orWhere('business.your_best_engineer', 'LIKE', "%{$search}%");
                });
            }
...
//        try {
        ...
        if (isset($data)) {
            ...
            $business_data = $business_data->map(function ($business_data) {
                ...
                $companies = Categories::whereIn('id', $companies_Ids)->get(['id', 'name']);
                ...
                $companiesNames = $companies->map(function ($company) {
                    return [
                        'id' => $company->id,
                        'value' => $company->name,
                    ];
                ...
                // Add new keys
                $business_data['companies_name'] = $companiesNames;
