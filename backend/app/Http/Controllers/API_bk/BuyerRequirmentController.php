...
//            $limit = $request->limit;
    ...
    public function requirment_post_by_user_id_post(Request $request) {
        ...
        try {
            ...
            $query = BuyerRequirment::select('register.role_id', BuyerRequirment::raw("IF(buyer_requirment_likes.status = 1, 1, 0) as is_likes"), 'buyer_requirment.id','buyer_requirment.product_id','buyer_requirment.role_id as roles_id', 'buyer_requirment.other_details', 'buyer_requirment.user_id', BuyerRequirment::raw("
            CASE 
                WHEN register.image IS NOT NULL AND register.image != '' 
                THEN CONCAT('https://developerwork.in/soundwale/public/storage/app/register/', register.image)
                ELSE CONCAT('https://developerwork.in/soundwale/public/admin-asset/images/profile_default_image.png') 
    ...
    public function PostUpdate(Request $request) {
        ...
        try {
            ...
            if ($request->type == "buyer") {
                ...
                if ($exist_buyer) {
                    $data = $request->only('role_id','country_id', 'other_details', 'state_id', 'city_id', 'requirment_id', 'categories_id', 'category_id', 'sub_category_id', 'price', 'description', 'image', 'status');
                    $categories_id = $request->categories_id;
                    if (isset($categories_id)) {
                        if (is_numeric($categories_id)) {
                            $categories_id = $categories_id;
