        ...
        <div class="kt-portlet__head kt-portlet__head--lg">
            ...
            <div class="kt-portlet__head-toolbar">
                    ...
                    <div class="kt-portlet__head-actions">
                        ...
                        &nbsp;
                        <a href="{{ route('admin.browse.by.positions.add') }}" class="btn btn-brand btn-elevate btn-icon-sm" id="add-service-form-modal">
    ...
    <script>
        let conf = {
            columns: [
                ...
                {
                    data: 'status', name: 'status', orderable:false, searchable:false, sClass:'text-center',
                    render:function(data,type,row){
                        var checked = (row.status == 1 ) ? 'checked' : '';
                        return '<span class="kt-switch kt-switch--outline kt-switch--icon kt-switch--success">'+
                                '<label>'+
                                    '<input type="checkbox" class="amenities-status-switch" '+checked+'  value="1" data-id="'+row.id+'">'+
                                    '<span></span>'+
                                '</label>'+
                            '</span>'
                    }
        ...
        makeDataTable('#amenities_list_table', "{{ route('admin.browse.by.positions.list') }}", conf);

        function updateStatus(newStatus,checkbox,recordId) {
            var checkbox = checkbox;
            $.ajax({
                url : "{{ route('admin.browse.by.positions.status.change') }}",
                type : 'POST',
