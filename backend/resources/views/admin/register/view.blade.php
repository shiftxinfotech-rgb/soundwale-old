        ...
        <div class="kt-portlet__body">
            <div class="row">
                ...
                <?php if ($data->role_id == "6") { ?>
                    <div class="col-lg-6">
                        <label><strong id="common_name_title">Your Shop Name:</strong></label>
...
//                            if ($mobile_num_row['type'] != "Owner") {
                            ...
                            <div class="col-lg-4">
                                ...
                                <p>{{ !empty($mobile_num_row['name']) ? $mobile_num_row['name'] : 'N/A' }}</p>
                            </div>
                            <div class="col-lg-4">
                                <label><strong>Mobile number</strong></label>
                                <p>{{ !empty($mobile_num_row['mobile_number']) ? ($mobile_num_row['code'] ?? '') . ' ' . $mobile_num_row['mobile_number'] : 'N/A' }}</p>
                            </div>
                            <div class="col-lg-4">
                                <label><strong>Email</strong></label>
                                <p>{{ !empty($mobile_num_row['email']) ? $mobile_num_row['email'] : 'N/A' }}</p>
                            </div>
                            <?php
//                            }
                        }
                ...
                $ids = explode(',', $business_data->category_id);
                ...
                <div class="col-lg-6">
                    ...
                    <p>{{ !empty($names) ? implode(', ', $names) : 'N/A' }}</p>
                </div>
                @endif
                <div class="col-lg-6">
        ...
        <div class="kt-portlet__body" id="gallery_div" style="display: none;">
            ...
            <div class="row">
                @if(!empty($business_shop_images_data) && count($business_shop_images_data) > 0)
        ...
        <div class="kt-portlet__body" id="product_information_div" style="display: none;">
            ...
            @php
            $convert_json_row = json_encode($row);
            @endphp
            <div class="row mb-3 form-group-group">
                <div class="form-group col-lg-4">
                    <label>Product</label>
                    <select name="selections[{{ $i }}][product_id]" class="form-control" disabled>
            ...
            <div class="form-group col-lg-4">
                <label>Company</label>
                <select name="selections[{{ $i }}][company_id]" class="form-control" disabled>
        ...
        <div class="form-group col-lg-3">
            <label>Model</label>
            <select name="selections[{{ $i }}][model_id]" class="form-control" disabled>
    ...
    <div class="form-group col-lg-1 d-flex align-items-end">
        <input type="hidden" id="product_delete_json_{{ $i }}" name="product_delete_json_{{ $i }}" value="{{ $convert_json_row }}">
        <button type="button" class="btn btn-danger" onclick="delete_json_func('RemoveProduct',{{ $i }});">X</button>   
    </div>
</div>
@endforeach
@else
...
<div class="kt-portlet__body" id="working_with_div" style="display: none;">
    ...
    <div class="kt-portlet__body" id="working-with-wrapper">
        ...
        @php
        $convert_json_row = json_encode($workingWithRow);
        @endphp
        <div class="form-group col-lg-12 d-flex mb-2">
            <input type="text" name="working_with[]" value="{{ $workingWithRow['value'] }}" placeholder="Enter Working With" class="form-control mr-2" required disabled>
            <input type="hidden" id="working_with_delete_json_{{ $key }}" name="working_with_delete_json_{{ $key }}" value="{{ $convert_json_row }}">
            <button type="button" class="btn btn-danger" onclick="delete_json_func('RemoveWorkingWith',{{ $key }});">X</button>
        </div>
        @endforeach
        @else
        <span class="no-records-found">No records found</span>
...
<div class="kt-portlet__body" id="parts_div" style="display: none;">
    ...
    @php
    $convert_json_row = json_encode($row_part);
    @endphp
    <div class="row mb-3 form-group-group-part">
        <div class="form-group col-lg-4">
            <label>Part Name</label>
            <select name="selections_part[{{ $j1 }}][parts_id]"  class="form-control" disabled>
                <option value="">Select Product</option>
    ...
    <div class="form-group col-lg-4">
        <label>Company</label>
        <select name="selections_part[{{ $j1 }}][company_id]"  class="form-control" disabled>
            <option value="">Select Company</option>
            @foreach($categories as $company)
...
<div class="form-group col-lg-3">
    <label>Details</label>
    <input type="text" name="selections_part[{{ $j1 }}][details]"  value="{{ $row_part['details'] }}" placeholder="Enter Details" class="form-control mr-2" disabled>
    </select>
</div>
<div class="form-group col-lg-1 d-flex align-items-end">
    <input type="hidden" id="parts_delete_json_{{ $j1 }}" name="parts_delete_json_{{ $j1 }}" value="{{ $convert_json_row }}">
    <button type="button" class="btn btn-danger" onclick="delete_json_func('RemoveParts',{{ $j1 }});">X</button>   
    <!--remove-row-part-->
</div>
</div>
@endforeach
...
<div class="kt-portlet__body" id="service_center_div" style="display: none;">
    ...
    <div class="row mb-3 form-group-service">
        <div class="form-group col-lg-2">
            <label>Name</label>
            <input type="text" name="selections_service[{{ $j }}][center_name]" value="{{ $row['center_name'] }}" placeholder="Enter Name" class="form-control mr-2" disabled>
        </div>
        <div class="form-group col-lg-3">
            <label>Company</label>
            <select name="selections_service[{{ $j }}][company_id]" class="form-control" disabled>
                <option value="">Select Company</option>
                @foreach($categories as $company)
                <option value="{{ $company->id }}" @if($company->id == $row['company_id']) selected @endif>
                        {{ $company->name }}
    ...
    <div class="form-group col-lg-3">
        ...
        @php
        $fullMobile = ($row['code'] ?? '') . ($row['mobile_number'] ?? '');
        $fullMobile = str_replace(' ', '', $fullMobile);
        @endphp
        <input type="text" id="" name="selections_service[{{ $j }}][mobile_number]" disabled data-validation="required|number" value="{{ $fullMobile ?? null }}" class="form-control only-mobile" placeholder="">
    ...
    <div class="form-group col-lg-3">
        ...
        <input type="text" name="selections_service[{{ $j }}][location]" disabled  value="{{ $row['location'] }}" id="location_{{ $j }}" onclick="OpenMapPopup({{ $j }});" placeholder="Enter Location" class="form-control mr-2" required>
    ...
    <div class="form-group col-lg-1 d-flex align-items-end">
        <input type="hidden" id="service_center_delete_json_{{ $j }}" name="service_center_delete_json_{{ $j }}" value="{{ $convert_json_row }}">
        <button type="button" class="btn btn-danger" onclick="delete_json_func('RemoveServiceCenter',{{ $j }});">X</button>   
    </div>
</div>
@endforeach
@else
...
<div class="kt-portlet__body" id="product_information_multi_div" style="display: none;">
    ...
    <div class="kt-portlet__body">
        <div class="row">
            <div class="form-group col-lg-4">
                ...
                <select name="companies_id[]" class="form-control select2" multiple disabled>
                    @foreach($categories as $company)
        ...
        <div class="form-group col-lg-4">
            ...
            <select name="category_id[]" id="category_id" class="form-control select2" multiple disabled>
                @foreach($category as $product)
    ...
    <div class="form-group col-lg-4">
        ...
        <select name="sub_category_id[]" id="sub_category_id" class="form-control select2" multiple disabled>
...
<div class="kt-portlet__body" id="top_graduates_div" style="display: none;">
    ...
    <!--    <form class="kt-form" action="{{ route('admin.register.store_view') }}" id="user-gallery-from" method="post" isReload="{{ route('admin.register.view', ['id' => $data->id]) }}"  enctype="multipart/form-data">
            ...
            <input type="hidden" name="form_type" value="WorkingWith">-->
    @php
    $workingWithList = json_decode($business_data->your_best_engineer ?? '[]', true);
    @endphp
    ...
    <div class="kt-portlet__body" id="graduates-with-wrapper">
        ...
        @endphp
        <div class="form-group col-lg-12 d-flex mb-2">
            <input type="text" name="working_with[]" value="{{ $workingWithRow['value'] }}" placeholder="Enter Name" class="form-control mr-2" disabled>
            <input type="hidden" id="student_delete_json_{{ $key }}" name="student_delete_json_{{ $key }}" value="{{ $convert_json_row }}">
            <button type="button" class="btn btn-danger" onclick="delete_json_func('RemoveStudent',{{ $key }});">X</button>   
        </div>
        @endforeach
        @else
        <span class="no-records-found">No records found</span>
...
<div class="kt-portlet__body">
    ...
    <table>
        ...
        <tr id="delete_review_div_{{ $review_data_row->id }}">
            ...
            <td>{{ $review_data_row->register_email ?? 'N/A' }}</td>
...
@push('script')
<script>
    function delete_json_func(type, counter){
    var user_id = $("#user_id").val();
    if (type == "RemoveParts"){
    var delete_object_json = $("#parts_delete_json_" + counter).val();
    } else if (type == "RemoveServiceCenter"){
    var delete_object_json = $("#service_center_delete_json_" + counter).val();
    } else if (type == "RemoveProduct"){
    var delete_object_json = $("#product_delete_json_" + counter).val();
    } else if (type == "RemoveWorkingWith"){
    var delete_object_json = $("#working_with_delete_json_" + counter).val();
    } else if (type == "RemoveStudent"){
    var delete_object_json = $("#student_delete_json_" + counter).val();
    }
    if (confirm("Are you sure you want to remove this?")) {
    $.ajax({
    url: "{{ route('admin.register.json_object_remove') }}",
            type: 'POST',
            data: {
            _token: '{{ csrf_token() }}',
                    delete_object_json: delete_object_json,
                    type: type,
                    user_id: user_id,
            },
...
<script>
    ...
    if (confirm("Are you sure you want to remove this?")) {
    $.ajax({
    url: "{{ route('admin.register.delete_user_profile') }}",
            type: 'POST',
            data: {
            _token: '{{ csrf_token() }}',
                    id: imageId
            ...
            success: function (response) {
            if (response.success) {
            $('#delete_user_profile_div').remove(); // remove image from DOM
            } else {
            alert('Failed to delete image.');
            }
            }
