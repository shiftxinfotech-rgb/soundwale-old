...
window.validationChk = (element,rules = null) => {
    let defaultConfig = {
        focusInvalid: true,
...
window.makeDataTable = function (element, route, config, extraFilter = null) {
...
if ($('.select-nice').length) {
    $('.select-nice').select2({placeholder: 'Select'});
}

//Nice select
if ($('.select-nice-tag').length) {
    $('.select-nice-tag').select2({placeholder: 'Select'});
}
